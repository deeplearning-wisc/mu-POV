...
def get_agents(args, peft_path=None):
    ...
    The agent exposes `.tokenizer`, `.huggingface_model`, and `.args`.
    For ModeX-Lite (--new_decode), the wrapper also provides a custom
    ``.generate()`` that performs online similarity-based pruning via
    ``model/ma_decoder.py``.
    """
