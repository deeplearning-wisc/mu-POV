...
def get_responses(question, agent, num_agents, personas, suffix):
...
def goodness_of_cut(args, A, group_indices, n_group_indices):
    """Compute a graph cut quality metric for early stopping."""
...
def vote(args, question, answer, agent_responses, agent, num_agents, personas, suffix, evaluate_func):
    """ModeX post-hoc selection via recursive spectral graph cuts."""
    ...
    while True:
        ...
        if len(info['groups']['group_1']) > len(info['groups']['group_2']):
            ...
            if np.sum(degrees[g1_idx]) >= np.sum(degrees[g2_idx]):
                group, n_group = info['groups']['group_1'], info['groups']['group_2']
            else:
                group, n_group = info['groups']['group_2'], info['groups']['group_1']
...
def graph_cut(args, A, agent_names):
    """Spectral bi-partition via the Fiedler vector."""
...
def run_experiment(args, test_X, test_Y, agent, personas, exp_name):
    ...
    for i, (x, y) in enumerate(tqdm(zip(test_X, test_Y), total=len(test_X))):
        ...
        prompts, agent_responses, responses = get_responses(
            x, agent, args.num_agents, personas, suffix
        )

        # ModeX-Lite: when --new_decode is set, online pruning during generation has
        # already reduced the batch; evaluate all surviving responses directly.
        voted_response = agent_responses
        depth = 0
...
def main(args):
    ...
    if args.prune_frequency > 0:
        exp_name += f"_PF={args.prune_frequency}"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ModeX-Lite: Online-pruning decoding and baseline comparisons"
    )
    ...
    parser.add_argument('--model_dir', type=str, default=None,
                        help="Local directory for model weights (None = HuggingFace Hub)")
    ...
    # Similarity graph
    parser.add_argument('--adjacency', type=str, default='text', choices=['text', 'semantics', 'both'])
    ...
    # ModeX-Lite specific
    parser.add_argument('--new_decode', action='store_true',
                        help="Enable ModeX-Lite online pruning during token generation")
    parser.add_argument('--prune_frequency', type=int, default=100,
                        help="Token interval between pruning steps (ModeX-Lite only)")
