...
def engine(messages, agent, num_agents=1, stop_sequences=None):
    """
    Run batched generation for a list of chat messages and return responses.

    Args:
        messages: List of chat message dicts (or list of lists for multi-turn).
        agent: A model wrapper exposing `.tokenizer` and `.huggingface_model`.
        num_agents: Number of sequences to decode (used for display only).
    ...
    # Per-model sampling hyperparameters
    if 'Phi' in type(agent).__name__:
    ...
    outputs = agent.huggingface_model.generate(
        ...
        max_new_tokens=3072 if 'Phi' in type(agent).__name__ else 2048,
